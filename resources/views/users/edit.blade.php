...
<div class="flex items-center justify-between mb-0">
    <h1 class="font-bold text-gray-900 dark:text-gray-100">📝 Edit User</h1>
...
<div x-data="{ tab: 'personal' }">
    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data"
        class="mx-auto space-y-6 text-light">
        @csrf
        @method('PUT')

        {{-- Basic User Info --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-0">
            {{-- First Name --}}
            <div>
                ...
                <input type="text" name="first_name" id="first_name" required
                    ...
                    value="{{ old('first_name', $user->personal['first_name'] ?? '') }}">
            </div>
            ...
            <div>
                ...
                <input type="text" name="last_name" id="last_name" required
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2"
                    value="{{ old('last_name', $user->personal['last_name'] ?? '') }}">
            </div>
            ...
            <fieldset class="border p-3 rounded">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" name="personal[dob]"
                            value="{{ old('personal.dob', $user->personal['dob'] ?? '') }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    </div>
                    <div>
                        <label>Gender</label>
                        <select name="personal[gender]"
                            ...
                            <option value="Male" @selected(old('personal.gender', $user->personal['gender'] ??
                                '')=='Male')>Male</option>
                            <option value="Female" @selected(old('personal.gender', $user->personal['gender'] ??
                                '')=='Female')>Female</option>
                            <option value="Other" @selected(old('personal.gender', $user->personal['gender'] ??
                                '')=='Other')>Other</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="personal[maritalStatus]"
                            ...
                            <option value="Single" @selected(old('personal.maritalStatus', $user->
                                personal['maritalStatus'] ?? '')=='Single')>Single</option>
                            <option value="Married" @selected(old('personal.maritalStatus', $user->
                                personal['maritalStatus'] ?? '')=='Married')>Married</option>
                            <option value="Other" @selected(old('personal.maritalStatus', $user->
                                personal['maritalStatus'] ?? '')=='Other')>Other</option>
                        </select>
                    </div>
                    <div>
                        <label>Blood Group</label>
                        <input type="text" name="personal[bloodGroup]"
                            value="{{ old('personal.bloodGroup', $user->personal['bloodGroup'] ?? '') }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    </div>
                    <div>
                        <label>Nationality</label>
                        <input type="text" name="personal[nationality]"
                            value="{{ old('personal.nationality', $user->personal['nationality'] ?? '') }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    </div>
                    <div>
                        <label for="profile_photo" class="block font-medium text-gray-700 dark:text-gray-200">Profile
                            Photo</label>
                        ...
                        <div class="mt-3 flex justify-start">
                            <img id="photoPreview"
                                src="{{ $user->profile_photo ? asset('storage/' . $user->profile_photo) : 'https://via.placeholder.com/100x100?text=Preview' }}"
                                alt="Preview"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="contact[phone]"
                            value="{{ old('contact.phone', $user->contact['phone'] ?? '') }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    </div>
                    <div>
                        <label>Alternate Phone</label>
                        <input type="text" name="contact[altPhone]"
                            value="{{ old('contact.altPhone', $user->contact['altPhone'] ?? '') }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    </div>
                    <div>
                        <label>Address</label>
                        <textarea name="contact[address]"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">{{ old('contact.address', $user->contact['address'] ?? '') }}</textarea>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    <div>
                        ...
                        <input type="text" name="contact[city]"
                            value="{{ old('contact.city', $user->contact['city'] ?? '') }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    </div>
                    <div>
                        <label>State</label>
                        <input type="text" name="contact[state]"
                            value="{{ old('contact.state', $user->contact['state'] ?? '') }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    </div>
                    <div>
                        <label>Pincode</label>
                        <input type="text" name="contact[pincode]"
                            value="{{ old('contact.pincode', $user->contact['pincode'] ?? '') }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    </div>
                </div>
            </fieldset>
    ...
    <fieldset class="border p-3 rounded">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="text" name="official[employeeCode]"
                    value="{{ old('official.employeeCode', $user->official['employeeCode'] ?? '') }}" readonly
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
            </div>
            <div>
                ...
                <select id="departmentSelect" name="official[department]"
                    ...
                    @foreach ($departments as $department)
                        <option value="{{ $department->id }}" 
                            @selected(old('official.department', $user->official['department'] ?? '') == $department->id)>
                            {{ $department->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select id="designationSelect" name="official[designation]"
                    ...
                    @foreach ($designations as $designation)
                        <option value="{{ $designation->id }}" 
                            @selected(old('official.designation', $user->official['designation'] ?? '') == $designation->id)>
                            {{ $designation->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <div>
                ...
                <input type="date" name="official[joiningDate]"
                    value="{{ old('official.joiningDate', $user->official['joiningDate'] ?? '') }}"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
            </div>
            <div>
                <label>Location</label>
                <input type="text" name="official[location]"
                    value="{{ old('official.location', $user->official['location'] ?? '') }}"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
            </div>
            <div>
                ...
                <select name="official[employmentType]"
                    ...
                    <option value="permanent" @selected(old('official.employmentType', $user->official['employmentType'] ?? '') == 'permanent')>Permanent</option>
                    <option value="contract" @selected(old('official.employmentType', $user->official['employmentType'] ?? '') == 'contract')>Contract</option>
                    <option value="intern" @selected(old('official.employmentType', $user->official['employmentType'] ?? '') == 'intern')>Intern</option>
                </select>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <div>
                ...
                <select name="official[role]"
                    ...
                    @foreach ($roles as $role)
                        <option value="{{ $role->name }}" class="uppercase" 
                            @selected(old('official.role', $user->official['role'] ?? '') == $role->name)>
                            {{ $role->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select id="parentUserSelect" name="parent_id" placeholder="Type to search..." class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
                    ...
                    @foreach ($parentUsers as $pUser)
                        <option value="{{ $pUser->id }}" 
                            @selected(old('parent_id', $user->parent_id ?? '') == $pUser->id)>
                            {{ $pUser->name }} ({{ $pUser->roles->pluck('name')->join(', ') }})
                        </option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="official[status]"
                    ...
                    <option value="active" @selected(old('official.status', $user->official['status'] ?? '') == 'active')>Active</option>
                    <option value="inactive" @selected(old('official.status', $user->official['status'] ?? '') == 'inactive')>Inactive</option>
                    <option value="suspended" @selected(old('official.status', $user->official['status'] ?? '') == 'suspended')>Suspended</option>
                </select>
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            <input type="date" name="official[lastWorkingDate]" 
                value="{{ old('official.lastWorkingDate', $user->official['lastWorkingDate'] ?? '') }}"
                class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
        </div>
    </fieldset>
</div>
    ...
    <fieldset class="border p-3 rounded">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="text" name="payroll[bankName]" value="{{ old('payroll.bankName', $user->payroll['bankName'] ?? '') }}"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
            </div>
            <div>
                <label>Account Number</label>
                <input type="text" name="payroll[accountNumber]" value="{{ old('payroll.accountNumber', $user->payroll['accountNumber'] ?? '') }}"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
            </div>
            <div>
                <label>IFSC</label>
                <input type="text" name="payroll[ifsc]" value="{{ old('payroll.ifsc', $user->payroll['ifsc'] ?? '') }}"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <div>
                <label>PAN</label>
                <input type="text" name="payroll[pan]" value="{{ old('payroll.pan', $user->payroll['pan'] ?? '') }}"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
            </div>
            <div>
                <label>Salary</label>
                <input type="number" name="payroll[salary]" value="{{ old('payroll.salary', $user->payroll['salary'] ?? '') }}"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2" step="0.01">
            </div>
            <div>
                <label>Last Salary Update</label>
                <input type="date" name="payroll[lastSalaryUpdate]" value="{{ old('payroll.lastSalaryUpdate', $user->payroll['lastSalaryUpdate'] ?? '') }}"
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            <input type="text" name="payroll[costCenter]" value="{{ old('payroll.costCenter', $user->payroll['costCenter'] ?? '') }}"
                class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">
        </div>
    </fieldset>
</div>
        ...
        <div x-show="tab === 'experience'">
            ...
            <button type="submit" class="px-6 py-2 bg-success text-white rounded hover:bg-indigo-700 mt-2">Update
                User</button>
        </div>
    </form>
</div>
...
document.getElementById('add-education-btn').addEventListener('click', () => {
    educationContainer.insertAdjacentHTML('beforeend', getEducationTemplate(educationCount));
    educationCount++;
});

document.getElementById('add-experience-btn').addEventListener('click', () => {
    experienceContainer.insertAdjacentHTML('beforeend', getExperienceTemplate(experienceCount));
    experienceCount++;
});
...
// On page load: populate existing data
window.onload = () => {
    const oldEducation = @json(old('education', $user -> education ?? []));
    const oldExperience = @json(old('experience', $user -> experience ?? []));

    if (oldEducation.length > 0) {
        oldEducation.forEach(ed => {
            educationContainer.insertAdjacentHTML('beforeend', getEducationTemplate(educationCount, ed));
            educationCount++;
        });
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateEmail() {
        ...
        const generatedEmail = `${f}.${l}@bvinfy.com`;

        // Only overwrite if email is empty or was previously auto-generated
        if (f && l && (!currentEmail || currentEmail.endsWith('@bvinfy.com'))) {
    ...
    // ---------- 2. Dynamic designation dropdown ----------
    const departmentSelect = document.getElementById('departmentSelect');
    const designationSelect = document.getElementById('designationSelect');
    const currentDesignation = designationSelect?.dataset.current || '';

    function loadDesignations(departmentId, preselectId = '') {
        ...
        if (departmentId) {
            fetch(`/departments/${departmentId}/designations`)
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        const selected = item.id == preselectId ? 'selected' : '';
    ...
    // Load designations on page load if a department is already selected
    if (departmentSelect.value) {
        loadDesignations(departmentSelect.value, currentDesignation);
    }
